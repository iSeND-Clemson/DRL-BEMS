...
def callback_function_DQN(state_argument):
    ...
    if EPLUS.RL_flag == True:
        ...
        if EPLUS.time_interval == 0:
            ...
            EPLUS.episode_reward.append(0)
            EPLUS.action_list.append([0,0,0,0,0,0])
        ...
        action_1 = [agent.select_action(state_1[i]) for i, agent in enumerate(agents)]
        ...
        # 4 Smootheness
        current_action = EPLUS.action_list[-1]
        last_action    = EPLUS.action_list[-2]
        ...
        EPLUS.T_Violation_new.append(T_violation)
        ...
        for i, agent in enumerate(agents):
            agent.replay_buffer.add(state_0[i], action_0[i], reward_1[i], state_1[i])
        ...
        '''
        if agents[0].replay_buffer.size() > 2050:
            all_actor_losses = []
            for i, agent in enumerate(agents):

                b_s, b_a, b_r, b_ns = agent.replay_buffer.sample(64)
                transition_dict = {
                                    'states': b_s,
                                    'actions': b_a,
                                    'next_states': b_ns,
                ...
                # agent.add_experience(transition_dict)
                actor_loss = agent.update(transition_dict)
    ...
    if EPLUS.time_interval > 2050 and time_interval % EPLUS.FPS == 0:
        ...
        ax1.axhline(y=77, label='Upper comfort band', color='b', linewidth=2)
        ax1.axhline(y=68, label='Lower comfort band', color='b', linewidth=2)
        ax1.axhline(y=90, label='Upper Hard Constraint', color='r', linewidth=2)
        ax1.axhline(y=60, label='Lower Hard Constraint', color='r', linewidth=2)
        ...
        ax1.plot(x,Humd_mean, label='Zone_humd_mean', color='cyan', linewidth=7)
...
class ReplayBuffer:
    ...
    def add(self, state, action, reward, next_state):  # add data to buffer
...
class MADDPGAgent:
    ...
    def update(self, transition_dict):
        ...
        q_values = self.actor(states).gather(1, actions)  # Q value
        max_next_q_values = self.actor_target(next_states).max(1)[0].view(-1, 1)
        q_targets = rewards + self.gamma * max_next_q_values

        dqn_loss = torch.mean(F.mse_loss(q_values, q_targets))  # MSE
        self.actor_optimizer.zero_grad()  # clear grad
        dqn_loss.backward()  # back-propagation & update network
        self.actor_optimizer.step()

        if self.count % self.target_update == 0:
            self.actor_target.load_state_dict( 
                self.actor.state_dict())  # update target network
        self.count += 1
...
for epoch in range(epochs):
    ...
    Total_signal_change = np.sum(EPLUS.signal)
    ...
    Benchmark[epoch, 16] = H_Violation
    Benchmark[epoch, 17] = Total_signal_change
    Benchmark[epoch, 18] = EPLUS.Humd_mean
