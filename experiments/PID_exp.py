...
def callback_function_DQN(state_argument):
    ...
    if EPLUS.RL_flag == True:
        ...
        T1_pred = T1_PID.control(W1, E_loss, set_temp, T_11, zone_humd_2001)
        T2_pred = T2_PID.control(W1, E_loss, set_temp, T_21, zone_humd_2002)
        T3_pred = T3_PID.control(W1, E_loss, set_temp, T_31, zone_humd_2003)
        T4_pred = T4_PID.control(W1, E_loss, set_temp, T_41, zone_humd_2004)
        T5_pred = T5_PID.control(W1, E_loss, set_temp, T_51, zone_humd_2005)
        T6_pred = T6_PID.control(W1, E_loss, set_temp, T_61, zone_humd_2006)
        
        action_1 = [T1_pred, T2_pred, T3_pred, T4_pred, T5_pred, T6_pred]
        ...
        for humd in Humd_list:
            if 20< humd < 80:
                reward_H.append(1*work_flag)
                
            elif humd > 80:
                reward_H.append(80-Humd_mean)
            elif humd < 20:
                reward_H.append(Humd_mean-20)
        
            
        reward_H = np.mean(reward_H)
        ...
        # Total reward
        reward_1 = reward_T + reward_E + reward_H#+ reward_signal
    ...
    if EPLUS.time_interval > view_distance:
        ...
        Humd_mean = EPLUS.Humd_mean[-view_distance::]
    ...
    if EPLUS.time_interval > 2050 and time_interval % EPLUS.FPS == 0:
        ...
        ax1.plot(x,y_outdoor, label='Outdoor_temperature', color='orange', linewidth=3)
        ...
        ax1.axhline(y=77, label='Comfort Zone', color='darkgreen', linewidth=2)
        ax1.axhline(y=68, label='Comfort Zone', color='darkgreen', linewidth=2)
        ...
        ax1.plot(x,T_mean, label='Zone_temperature_mean', color='violet', linewidth=7)
        ax1.plot(x,y_zone_temp_2001, label='Zone_1_temperature', color='C0', linewidth=1)
        ax1.plot(x,y_zone_temp_2002, label='Zone_2_temperature', color='C1', linewidth=1)
        ax1.plot(x,y_zone_temp_2003, label='Zone_3_temperature', color='C2', linewidth=1)
        ax1.plot(x,y_zone_temp_2004, label='Zone_4_temperature', color='C3', linewidth=1)
        ax1.plot(x,y_zone_temp_2005, label='Zone_5_temperature', color='C4', linewidth=1)
        ax1.plot(x,y_zone_temp_2006, label='Zone_6_temperature', color='C5', linewidth=1)
        
        
        
        ax1.plot(x,Humd_mean, label='Zone_humidity_mean', color='cyan', linewidth=7)
        ax1.plot(x,y_zone_humd_2001, label='Zone_1_humidity', color='C0', linewidth=1)
        ax1.plot(x,y_zone_humd_2002, label='Zone_2_humidity', color='C1', linewidth=1)
        ax1.plot(x,y_zone_humd_2003, label='Zone_3_humidity', color='C2', linewidth=1)
        ax1.plot(x,y_zone_humd_2004, label='Zone_4_humidity', color='C3', linewidth=1)
        ax1.plot(x,y_zone_humd_2005, label='Zone_5_humidity', color='C4', linewidth=1)
        ax1.plot(x,y_zone_humd_2006, label='Zone_6_humidity', color='C5', linewidth=1)
        ...
        ax1.set_title('Zone Comfort and HVAC Energy Consumption', fontsize=20)
...
class PID:
    ...
    def control(self,W, E_loss, comfort, current_value, Humidity):

        if current_value < comfort[0]:
            T_error = (current_value - comfort[0])*(1-W) 
        elif current_value > comfort[1]:
            T_error = (current_value - comfort[1])*(1-W) 
        else:
            T_error = (current_value - 72)*(1-W) 
            
            
        if Humidity < 20:
            H_error = (Humidity - 20)*(W)
        elif Humidity > 80:
            H_error = (Humidity - 80)*(W)
        else:
            H_error = (Humidity - 50)*(W)
            
            
        error = T_error + E_loss * self.E_factor #+ H_error

            
        # error = (setpoint - current_value) 
        # error = E_loss * self.E_factor
...
T1_PID = PID(0.5, 0.01, 0.01)
T2_PID = PID(0.5, 0.01, 0.01)
T3_PID = PID(0.5, 0.01, 0.01)
T4_PID = PID(0.5, 0.01, 0.01)
T5_PID = PID(0.5, 0.01, 0.01)
T6_PID = PID(0.5, 0.01, 0.01)

# E_PID = PID(Kp_energy, Ki_energy, Kd_energy)
