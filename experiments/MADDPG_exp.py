...
def callback_function_DQN(state_argument):
    ...
    if EPLUS.RL_flag == True:
        ...
        state_0 = [
                        [O0/100, W0, T_10/100, H_10/100],
                        [O0/100, W0, T_20/100, H_20/100],
                        [O0/100, W0, T_30/100, H_30/100],
                        [O0/100, W0, T_40/100, H_40/100],
                        [O0/100, W0, T_50/100, H_50/100],
                        [O0/100, W0, T_60/100, H_60/100]
                  ]
        ...
        state_1 = [
                        [O1/100, W1, T_11/100, H_11/100],
                        [O1/100, W1, T_21/100, H_21/100],
                        [O1/100, W1, T_31/100, H_31/100],
                        [O1/100, W1, T_41/100, H_41/100],
                        [O1/100, W1, T_51/100, H_51/100],
                        [O1/100, W1, T_61/100, H_61/100]
                  ]
        
        
        
        # print(state_1)
        action_1 = [agent.select_action(state_1[i]).detach().cpu().numpy() for i, agent in enumerate(agents)]
        EPLUS.action_list.append(action_1)

        action_1 = [agent.select_action(state_1[i]).argmax().item() for i, agent in enumerate(agents)]
        EPLUS.action_map.append(action_1)
        ...
        # 4 Smootheness
        current_action = EPLUS.action_map[-1]
        last_action    = EPLUS.action_map[-2]
        ...
        reward_1 = [
                        [reward_T1 + reward_E/6],
                        [reward_T2 + reward_E/6],
                        [reward_T3 + reward_E/6],
                        [reward_T4 + reward_E/6],
                        [reward_T5 + reward_E/6],
                        [reward_T6 + reward_E/6]            
                    ]
        ...
        # transitions = {
        #                 'states': np.array(state_0),
        #                 'actions': np.array(action_0),
        #                 'rewards': np.array(reward_1),
        #                 'next_states': np.array(state_1),  # Replace with actual next states
        #                 'dones': np.zeros(num_agents)    # Replace with actual done flags
        #               }
        ...
        if EPLUS.time_interval > 2000:
            for i in range(num_agents):
                agents[i].add_experience(state_0[i], action_0[i], reward_1[i], state_1[i])

            if EPLUS.time_interval % 20 == 0:
                all_critic_losses = []
                all_actor_losses = []
                for agent in agents:
                    critic_loss, actor_loss = agent.update(64)
                    all_critic_losses.append(critic_loss)
                    all_actor_losses.append(actor_loss)
                
    
        #     for agent in agents:
        #         # print(agent.agent_index, EPLUS.time_interval)
        #         critic_loss, actor_loss = agent.update(transitions)
        #         all_critic_losses.append(critic_loss)
        #         all_actor_losses.append(actor_loss)
    
            # print(all_critic_losses, all_actor_losses)
    ...
    if EPLUS.time_interval > view_distance:
        ...
        hvac_htg_2001 = [1 if value > 0 else -1 for value in hvac_htg_2001]
        hvac_htg_2002 = [1 if value > 0 else -1 for value in hvac_htg_2002]
        hvac_htg_2003 = [1 if value > 0 else -1 for value in hvac_htg_2003]
        hvac_htg_2004 = [1 if value > 0 else -1 for value in hvac_htg_2004]
        hvac_htg_2005 = [1 if value > 0 else -1 for value in hvac_htg_2005]
        hvac_htg_2006 = [1 if value > 0 else -1 for value in hvac_htg_2006]
    ...
    if EPLUS.time_interval > 2050 and time_interval % EPLUS.FPS == 0:
        ...
        if EPLUS.RL_flag == True:
            ...
            print(action_map, np.sum(all_critic_losses), np.sum(all_actor_losses))
        ...
        ax1.axhline(y=77, label='Upper comfort band', color='b')
        ax1.axhline(y=68, label='Lower comfort band', color='b')
        ax1.axhline(y=90, label='Upper Hard Resrtict', color='r')
        ax1.axhline(y=60, label='Lower Hard Resrtict', color='r')
        ...
        ax1.plot(x,T_mean, label='Zone_temp_mean', color='violet', linewidth=5)
...
# Define the MADDPG Actor and Critic networks
...
class MADDPGAgent:
    ...
    def update(self, batch_size):
        ...
        # agent = all_agents[self.agent_index]

        # Compute the target Q-value
        target_q = self.critic_target(next_states, self.actor_target(next_states))
        target_q_values = rewards/100 + self.gamma * target_q
        q_values = self.critic(states, actions)

        # print(rewards)
        # print(q_values[i], target_q_values[i])
...
action_dim = 2  # Replace with your actual action dimension
learning_rate = 1e-3
gamma = 0.9
epsilon = 0
tau = 0.01
...
agents = [MADDPGAgent(i, 
                      ...
                      epsilon,
                      tau, 
                      device) for i in range(num_agents)]



# # Load agent weights
# for i, agent in enumerate(agents):
#     agent.actor.load_state_dict(torch.load(f"./weights/MADDPG_HVAC/agent_{i}_actor_weights.pth"))
#     agent.critic.load_state_dict(torch.load(f"./weights/MADDPG_HVAC/agent_{i}_critic_weights.pth"))
